  ...
  const StatusBadge = ({ status }) => {
    const getStatusColor = () => {
      switch (status?.toLowerCase()) {
        case 'open': return { bg: '#EEF2FF', text: '#4F46E5' };
    ...
    return (
      <View style={{ backgroundColor: colors.bg }} className="px-4 py-1 rounded-full">
            ...
            <View className="flex-1">
              ...
              <View className="flex-row items-center space-x-3">
                ...
                <View className="bg-indigo-50 px-3 py-1 rounded-full">
                  <Text className="text-indigo-600 font-medium">{issueDetails.issueTag}</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View className="bg-gray-50 rounded-xl p-4 space-y-4">
            <View className="flex-row items-center">
              ...
              <View className="ml-3">
                ...
                <Text className="text-gray-900 font-medium">{issueDetails.user.username}</Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center">
              ...
              <View className="ml-3">
                ...
                <Text className="text-gray-900 font-medium">{formatDate(issueDetails.reportedDate)}</Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center">
              ...
              <View className="ml-3">
                ...
                <Text className="text-gray-900 font-medium">
                  {issueDetails.location.city ? issueDetails.location.city : issueDetails.location.coordinates[1].toFixed(4)}, 
                  {issueDetails.location.country ? issueDetails.location.country : issueDetails.location.coordinates[0].toFixed(4)}
                </Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center">
              ...
              <View className="ml-3">
                ...
                <Text className="text-gray-900 font-medium">{formatDate(issueDetails.lastUpdated)}</Text>
              </View>
            </View>
          </View>
        </View>
          ...
          <View className="bg-white rounded-2xl p-6 shadow-sm mb-6">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {issueDetails.issuePhotos.map((photoUrl, index) => (
                <View key={index} className="mr-3 rounded-xl overflow-hidden shadow-sm">
                  <Image
                    source={{ uri: photoUrl }}
                    className="w-48 h-48"
                    resizeMode="cover"
        ...
        <View className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          >
            ...
            <Animated.View style={{ transform: [{ rotate }] }}>
              <MaterialIcons name="keyboard-arrow-down" size={24} color="#4B5563" />
            </Animated.View>
          </TouchableOpacity>
              ...
              {issueDetails.comments?.map((comment, index) => (
                <View key={index} className="bg-gray-50 rounded-xl p-4 mb-4">
                  <View className="flex-row items-center mb-3">
                    ...
                    <View className="ml-3">
                      <Text className="font-semibold text-gray-900">{comment.user}</Text>
                      <Text className="text-sm text-gray-500">2 hours ago</Text>
                    </View>
                  </View>
                  <Text className="text-gray-700 leading-relaxed">{comment.text}</Text>
                </View>
              ))}
            </View>
          )}
